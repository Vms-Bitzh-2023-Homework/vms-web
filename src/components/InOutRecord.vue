<template>
    ...
    <div style="margin: 0 30px 0 30px">
        <div>
            ...
            <div>
                ...
                />
                <el-date-picker
                    v-model="searchParkingCarData.value1"
                    type="date"
                    placeholder="选择日期"
        ...
        <div class="table_area">
            <el-table :data="inOutData" stripe style="width: 100%">
                ...
                <el-table-column prop="out_time" label="出场时间" width="200">
                    <template #default="{ row }">
                        <span>{{
                            FormatDate(new Date(row.out_time), "datetime", true)
                        }}</span>
                    </template>
                </el-table-column>
    ...
    <el-dialog v-model="editParkingCarVisible" title="修改出入时间信息">
        <el-form :model="editParkingCarFrom">
            ...
            <el-form-item label="入场时间" :label-width="120">
                <el-input v-model="editParkingCarFrom.in_time"></el-input>
            </el-form-item>
            <el-form-item label="出场时间" :label-width="120">
                <el-input v-model="editParkingCarFrom.out_time"></el-input>
            </el-form-item>
        </el-form>
        <template #footer>
    ...
    <el-dialog v-model="createParkingCarVisible" title="添加车辆信息">
        <el-form :model="createParkingCarFrom">
            ...
            <el-form-item label="入场时间" :label-width="120">
                <el-input v-model="createParkingCarFrom.in_time"></el-input>
            </el-form-item>
            <el-form-item label="出场时间" :label-width="120">
                <el-input v-model="createParkingCarFrom.out_time"></el-input>
            </el-form-item>
        </el-form>
        <template #footer>
            <span class="dialog-footer">
...
import {
    ...
    CreateParkingCar,
    InOutRespon,
} from "../types/InOutRecord.ts";
import router from "../router";

import { dayjs } from "element-plus";

// const value1 = ref('')
const inOutData = ref<InOutData[]>();
...
const searchParkingCarData = ref<InOutData>({
    id: 0,
    carNo: "",
    in_time: "",
    out_time: "",
...
const handleDateChange = (newDate: any) => {
    searchParkingCarData.value.value1 = newDate;
};
...
const searchParkingCar = () => {
    const params = {
        ...
        date: searchParkingCarData.value.value1
            ? dayjs(searchParkingCarData.value.value1).format("YYYY-MM-DD")
            : null,
    };
    http.post("/inOutRecord/inOutList", params)
        .then((res: any) => {
...
const showEditParkingCarModel = (parkingCarForm: UpdateParkingCar) => {
    ...
    editParkingCarFrom.out_time = FormatDate(
        new Date(parkingCarForm.out_time as string),
        "datetime",
        true
    );
...
const getParkingCar = () => {
    const params = {};
    http.post<InOutRespon>("/inOutRecord/inOutList", params)
        .then((res: InOutRespon) => {
...
// // 过滤已出场车辆
// const readParkingCarData = (data: InOutData[]) => {
//   const result = [];
